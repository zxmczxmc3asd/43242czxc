local a a={cache={}, load=function(b)if not a.cache[b]then a.cache[b]={c=a[b]()}end return a.cache[b].c end}do function a.a()return{
    ...
    function p.Drag(v, x, B)
        ...
        -- Cache de tween ativo
        local activeTween = nil
        
        -- Atualização com cancelamento de tween anterior
        local function update(L)
            if not F or not J.CanDraggable then return end
    
            -- CANCELA o tween anterior antes de criar novo
            if activeTween then
                activeTween:Cancel()
                activeTween = nil
            end
            ...
            -- Atualização DIRETA sem tween durante drag
        ...
        -- Armazena connections para limpeza
        local connections = {}
    
        for L, M in pairs(x) do
            local conn1 = M.InputBegan:Connect(function(N)
                if (N.UserInputType == Enum.UserInputType.MouseButton1 or 
                    ...
                    if C == nil then
                        ...
                        local conn = N.Changed:Connect(function()
            ...
            local conn2 = M.InputChanged:Connect(function(N)
                if F and C == M then
                    if N.UserInputType == Enum.UserInputType.MouseMovement or 
                       N.UserInputType == Enum.UserInputType.Touch then
                        update(N)
                    end
                end
            end)
        ...
        local conn3 = e.InputChanged:Connect(function(N)
            if F and C ~= nil then
                if N.UserInputType == Enum.UserInputType.MouseMovement or 
                   N.UserInputType == Enum.UserInputType.Touch then
                    update(N)
    ...
    local COLORS = {
        bg        = Color3.fromHex"#0f0f11",
        surface   = Color3.fromHex"#17171a",
        surfaceAlt= Color3.fromHex"#1e1e22",
        border    = Color3.fromHex"#2a2a30",
        accent    = Color3.fromHex"#6366f1",   -- indigo
        accentSoft= Color3.fromHex"#818cf8",
        textPrim  = Color3.fromHex"#f4f4f5",
        textSec   = Color3.fromHex"#71717a",
        textMuted = Color3.fromHex"#3f3f46",
        success   = Color3.fromHex"#22c55e",
        danger    = Color3.fromHex"#f87171",
        white     = Color3.new(1,1,1),
    }
    
    -- Helper: cria um separador horizontal fino
    ...
    -- Helper: cria um label estilizado
    local function Label(text, size, color, weight, parent)
        return ac("TextLabel",{
            Text=text,
            TextSize=size or 14,
            TextColor3=color or COLORS.textPrim,
            FontFace=Font.new(ab.Font, weight or Enum.FontWeight.Regular),
            BackgroundTransparency=1,
            AutomaticSize="XY",
            Size=UDim2.new(0,0,0,0),
            TextXAlignment="Left",
    ...
    function aa.new(ag, ah, ai, aj)
        ...
        local am = {}   -- instâncias das APIs
        local an        -- valor da key digitada
    
        -- Dimensões
        local WIDTH  = 420
        local THUMB_W = ag.KeySystem.Thumbnail and ag.KeySystem.Thumbnail.Image
                        and math.min((ag.KeySystem.Thumbnail.Width or 180), 200) or 0
    
        al.UIElements.Main.AutomaticSize = "Y"
        al.UIElements.Main.Size = UDim2.new(0, WIDTH + THUMB_W, 0, 0)
    
        -- ── Thumbnail lateral (opcional) ──────────────────────────
        local aq_thumb
        if ag.KeySystem.Thumbnail and ag.KeySystem.Thumbnail.Image then
            local thumbTitle
            if ag.KeySystem.Thumbnail.Title then
                thumbTitle = ac("TextLabel",{
                    Text = ag.KeySystem.Thumbnail.Title,
                    TextColor3 = COLORS.textPrim,
                    TextSize = 14,
                    FontFace = Font.new(ab.Font, Enum.FontWeight.SemiBold),
                    BackgroundTransparency = 1,
                    AutomaticSize = "XY",
            ...
            aq_thumb = ac("ImageLabel",{
                Image = ag.KeySystem.Thumbnail.Image,
                BackgroundColor3 = COLORS.surface,
                BackgroundTransparency = 0,
                Size = UDim2.new(0, THUMB_W, 1, 0),
        ...
        -- ── Container principal (lado direito / único) ─────────────
        local mainContainer = ac("Frame",{
            Size = UDim2.new(1, THUMB_W>0 and -THUMB_W or 0, 1, 0),
            Position = UDim2.new(0, THUMB_W, 0, 0),
            BackgroundTransparency = 1,
            Parent = al.UIElements.Main,
        },{
            ac("UIListLayout",{
        ...
        -- Ícone badge no header
        local iconBadge
        if ag.Icon then
            local iconImg = ab.Image(
                ag.Icon,
                ag.Title..":"..ag.Icon,
                6,
                "Temp",
        ...
        -- Bloco título + subtítulo no header
        local titleBlock = ac("Frame",{
            Size = UDim2.new(1, iconBadge and -54 or 0, 1, 0),
            Position = UDim2.new(0, iconBadge and 54 or 0, 0, 0),
            BackgroundTransparency = 1,
            Parent = header,
        },{
            ac("UIListLayout",{
        ...
        ac("TextLabel",{
            Text = "Key System · Authentication required",
            TextSize = 12,
        ...
        -- Linha accent no topo do header (decorativa)
        local accentLine = ac("Frame",{
            ...
            ac("UICorner",{ CornerRadius=UDim.new(0,99) }),
        })
    
        -- Animação: expande a linha ao abrir
        task.spawn(function()
            task.wait(0.15)
            ad(accentLine, 0.55, {Size=UDim2.new(1,-40,0,3)},
               Enum.EasingStyle.Quint, Enum.EasingDirection.Out):Play()
        end)
        ...
        if ag.KeySystem.Note and ag.KeySystem.Note ~= "" then
            ...
            -- ícone info
            local infoIcon = ab.Image("info","info",0,"Temp","KeySystem",true)
            infoIcon.Size = UDim2.new(0,14,0,14)
            infoIcon.ImageLabel.ImageColor3 = COLORS.accentSoft
            infoIcon.Parent = noteFrame
        ...
        -- Campo de input da key (estilizado manualmente)
        local inputWrapper = ac("Frame",{
        ...
        -- Outline do input (muda cor no foco)
        local inputOutline = ab.NewRoundFrame(10,"Glass-1",{
            Size = UDim2.new(1,0,1,0),
            ImageColor3 = COLORS.border,
            ImageTransparency = 0,
            Parent = inputWrapper,
            ZIndex = 0,
        })
    
        local keyIcon = ab.Image("key","key",0,"Temp","KeySystem",true)
        keyIcon.Size = UDim2.new(0,16,0,16)
        keyIcon.ImageLabel.ImageColor3 = COLORS.textSec
        keyIcon.Parent = inputWrapper
    
        local inputBox = ac("TextBox",{
        ...
        ab.AddSignal(inputBox.Focused, function()
            ad(inputOutline, 0.2, {ImageColor3=COLORS.accent, ImageTransparency=0.6}):Play()
            ad(inputWrapper, 0.2, {BackgroundColor3=COLORS.surfaceAlt}):Play()
            keyIcon.ImageLabel.ImageColor3 = COLORS.accentSoft
        end)
        ab.AddSignal(inputBox.FocusLost, function()
            ad(inputOutline, 0.2, {ImageColor3=COLORS.border, ImageTransparency=0}):Play()
            keyIcon.ImageLabel.ImageColor3 = COLORS.textSec
        end)
    
        ab.AddSignal(inputBox:GetPropertyChangedSignal"Text", function()
            an = inputBox.Text
        end)
    
        -- ── FOOTER (botões + get key) ──────────────────────────────
        local footer = ac("Frame",{
            Size = UDim2.new(1,0,0,0),
            AutomaticSize = "Y",
            BackgroundTransparency = 1,
            Parent = body,
        ...
        -- Botão Get Key (link/API)
        if ag.KeySystem.URL or ag.KeySystem.API then
    
            local getKeyBtn = ac("TextButton",{
            ...
            local linkIcon = ab.Image("external-link","external-link",0,"Temp","KeySystem",true)
            linkIcon.Size = UDim2.new(0,13,0,13)
            linkIcon.ImageLabel.ImageColor3 = COLORS.accentSoft
            ...
            ab.AddSignal(getKeyBtn.MouseEnter, function()
                ad(getKeyBtn,0.12,{BackgroundColor3=COLORS.accent,BackgroundTransparency=0.85}):Play()
            end)
            ab.AddSignal(getKeyBtn.MouseLeave, function()
                ad(getKeyBtn,0.12,{BackgroundColor3=COLORS.surfaceAlt,BackgroundTransparency=0}):Play()
            end)
            ab.AddSignal(getKeyBtn.MouseButton1Click, function()
                ...
                elseif ag.KeySystem.API then
                    for _, svc in next, ag.KeySystem.API do
                        local g = ag.WindUI.Services[svc.Type]
                        if g then
                            local h = {}
                            for _, l in next, g.Args do table.insert(h, svc[l]) end
                            g.New(table.unpack(h)).Copy()
                        end
                    end
                    ag.WindUI:Notify{
                        Title = "Key System",
        ...
        local exitIcon = ab.Image("x","x",0,"Temp","KeySystem",true)
        exitIcon.Size = UDim2.new(0,14,0,14)
        exitIcon.AnchorPoint = Vector2.new(0.5,0.5)
        exitIcon.Position = UDim2.new(0.5,0,0.5,0)
        exitIcon.ImageLabel.ImageColor3 = COLORS.textSec
        exitIcon.Parent = exitBtn
    
        ab.AddSignal(exitBtn.MouseEnter, function()
            ad(exitBtn,0.12,{BackgroundColor3=COLORS.danger,BackgroundTransparency=0.85}):Play()
            exitIcon.ImageLabel.ImageColor3 = COLORS.danger
        end)
        ab.AddSignal(exitBtn.MouseLeave, function()
            ad(exitBtn,0.12,{BackgroundColor3=COLORS.surfaceAlt,BackgroundTransparency=0}):Play()
            exitIcon.ImageLabel.ImageColor3 = COLORS.textSec
        end)
        ab.AddSignal(exitBtn.MouseButton1Click, function()
            al:Close()()
        end)
    
        -- Botão Submit
        local submitBtn = ac("TextButton",{
            Size = UDim2.new(0,0,0,36),
            AutomaticSize = "X",
            BackgroundColor3 = COLORS.accent,
            BackgroundTransparency = 0,
        ...
        local arrowIcon = ab.Image("arrow-right","arrow-right",0,"Temp","KeySystem",true)
        arrowIcon.Size = UDim2.new(0,14,0,14)
        arrowIcon.ImageLabel.ImageColor3 = Color3.new(1,1,1)
        arrowIcon.Parent = submitBtn
    
        ab.AddSignal(submitBtn.MouseEnter, function()
            ad(submitBtn,0.15,{BackgroundColor3=COLORS.accentSoft}):Play()
            ad(arrowIcon,0.15,{Position=UDim2.new(0,2,0,0)}):Play()
        end)
        ab.AddSignal(submitBtn.MouseLeave, function()
            ad(submitBtn,0.15,{BackgroundColor3=COLORS.accent}):Play()
            ad(arrowIcon,0.15,{Position=UDim2.new(0,0,0,0)}):Play()
        end)
    
        -- ── Lógica de sucesso / submit ─────────────────────────────
        local function handleSuccess(keyValue)
            -- Animação de sucesso: linha accent vira verde
            ad(accentLine, 0.3, {BackgroundColor3=COLORS.success}):Play()
            ad(submitBtn, 0.3, {BackgroundColor3=COLORS.success}):Play()
    
            task.spawn(function()
                task.wait(0.35)
                if al and al.Close then al:Close()() end
                local folder = (ag and ag.Folder) or "Temp"
                local keyFile = (ah and tostring(ah)) or "key"
                if writefile then
                    writefile(folder.."/"..keyFile..".key", tostring(keyValue))
                end
                task.wait(0.4)
                if ai then ai(true) end
            end)
        end
    
        ab.AddSignal(submitBtn.MouseButton1Click, function()
            if not ag or not ag.KeySystem then return end
            local keyVal = tostring(an or "")
    
            -- Animação de loading no botão
            ad(submitBtn, 0.1, {BackgroundTransparency=0.3}):Play()
            task.spawn(function()
                task.wait(0.1)
                ad(submitBtn, 0.1, {BackgroundTransparency=0}):Play()
            end)
    
            if ag.KeySystem.KeyValidator then
                local ok = ag.KeySystem.KeyValidator(keyVal)
                if ok then
                    if ag.KeySystem.SaveKey then
                        handleSuccess(keyVal)
                    else
                        if al and al.Close then al:Close()() end
                        task.wait(0.4)
                        if ai then ai(true) end
                    ...
                    ad(inputOutline, 0.15, {ImageColor3=COLORS.danger, ImageTransparency=0.5}):Play()
                    ad(inputWrapper, 0.15, {BackgroundColor3=Color3.fromHex"#2a1a1a"}):Play()
                    task.delay(1.5, function()
                        ad(inputOutline, 0.2, {ImageColor3=COLORS.border, ImageTransparency=0}):Play()
                        ad(inputWrapper, 0.2, {BackgroundColor3=COLORS.surfaceAlt}):Play()
                    end)
                    if ag.WindUI and ag.WindUI.Notify then
                        ag.WindUI:Notify{ Title="Invalid Key", Content="The key you entered is incorrect.", Icon="triangle-alert" }
                    end
                end
                
            elseif not ag.KeySystem.API then
                local keyRef = ag.KeySystem.Key
                local ok = type(keyRef)=="table"
                    and table.find(keyRef, keyVal)
                    or keyRef == keyVal
                ...
                -- API verification
                local verified, msg
                for _, svc in next, am or {} do
                    if type(svc.Verify) == "function" then
                        local ok, result = svc.Verify(keyVal)
                        if ok then verified, msg = true, result; break end
                        msg = result
                    end
                end
                if verified then
                    ...
                    if ag.WindUI and ag.WindUI.Notify then
                        ag.WindUI:Notify{ Title="Key System Error", Content=msg or "Verification failed.", Icon="triangle-alert" }
                    end
                end
            end
        end
        ...
        -- Registrar APIs
        if ag.KeySystem.API and ag.WindUI and ag.WindUI.Services then
            for _, svc in next, ag.KeySystem.API do
                local g = ag.WindUI.Services[svc.Type]
                if g and type(g.New) == "function" then
                    local h = {}
                    local args = g.Args or {}
                    for _, l in next, args do table.insert(h, svc[l]) end
    ...
    local function render()
        local now = tick()
        if now - lastRender < 0.033 then return end  -- ~30fps máximo
        lastRender = now
    ...
    local an=ab.Drag(al, {am})
    
    function ag.SetDraggable(ao,ap)
        if an then
            an:Set(ap~=false)
        end
    ...
    -- Configuração inicial do draggable
    local initialDraggable = af.Draggable
    if initialDraggable == nil then
        initialDraggable = true
    end
    
    if initialDraggable then
        an:Set(true)
    ...
    -- SUBSTITUIR A PARTE DO DRAGGABLE POR ISTO:
    if aq.Draggable==false then
        ag:SetDraggable(false)
    elseif aq.Draggable~=false then
        ag:SetDraggable(true)
    end
    ...
    local axImageLabel = al("ImageLabel",{
    ...
    local l=ak.Drag(
        ...
        function(l,m)
            if not au.Closed then
                ...
                -- ADICIONA ISSO:
                if l then
                    -- iniciou drag: esconde coisas pesadas
                    aA.Visible = false
                    if au.AcrylicPaint and au.AcrylicPaint.Model then
                        au.AcrylicPaint.Model.Transparency = 1
                    end
                else
                    -- terminou drag: restaura
                    aA.Visible = true
                    if au.AcrylicPaint and au.AcrylicPaint.Model then
                        au.AcrylicPaint.Model.Transparency = 0.98
                    end
                end
            end
        end
    ...
    task.wait(.45)
    if au.Resizable then
    am(axImageLabel,.45,{ImageTransparency=.8},Enum.EasingStyle.Exponential,Enum.EasingDirection.Out):Play()  -- SÓ ESTA LINHA
    au.CanResize=true
    end
    end)
    ...
    am(axImageLabel,.45,{ImageTransparency=.8},Enum.EasingStyle.Exponential,Enum.EasingDirection.Out):Play()
    l:Set(false)
    au.CanResize=false
    ...
    am(axImageLabel,0.1,{ImageTransparency=.35}):Play()
    
    ak.AddSignal(B.Changed,function()
    if B.UserInputState==Enum.UserInputState.End then
    isResizing=false
    ay.Active=false
    am(axImageLabel,0.17,{ImageTransparency=.8}):Play()
    end
    end)
    end
    end
